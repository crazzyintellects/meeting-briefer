...
class SummarizationModel(object):
    ...
    def _add_seq2seq(self):
        ...
        if hps.mode == "decode":
            ...
            if len(final_dists) != 1:  # final_dists is a singleton list containing shape (batch_size, extended_vsize)
                raise ValueError("final_dists should be a singleton list containing shape")
    ...
    """
        ...
        if len(results['attn_dists']) != 1:
            raise ValueError("len(results['attn_dists']) != 1")
        attn_dists = results['attn_dists'][0].tolist()

        if FLAGS.pointer_gen:
            # Convert singleton list containing a tensor to a list of k arrays
            if len(results['p_gens']) != 1:
                raise ValueError("len(results['p_gens']) != 1")
        ...
        if FLAGS.coverage:
            ...
            if len(new_coverage) != beam_size:
                raise ValueError("len(new_coverage) != beam_size")
        else:
