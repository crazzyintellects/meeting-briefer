...
PAD_TOKEN = '[PAD]'  # This has a vocab id, which is used to pad the encoder input, decoder input and target sequence # nosec - B105:hardcoded_password_string]
UNKNOWN_TOKEN = '[UNK]'  # This has a vocab id, which is used to represent out-of-vocabulary words # nosec - B105:hardcoded_password_string]
...
def example_generator(data_path, single_pass):
  ...
  """
    while True:
        ...
        if not filelist:
            raise ValueError('Error: Empty filelist at %s' % data_path)  # check filelist isn't empty
        if single_pass:
    ...
    for i in id_list:
        try:
            ...
            if article_oovs is None:
                raise ValueError(
                    "Error: model produced a word ID that isn't in the vocabulary. This should not happen in"
                    "baseline (no pointer-generator) mode")
